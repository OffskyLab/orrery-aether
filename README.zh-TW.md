...
- `aether client setup`：從 `CLAUDE.md`/manifest **推斷** description/capabilities 讓你確認（`--yes`/旗標可非互動）→ 只註冊**自己這個 body**（additive、fail-closed）→ ping Redis。body 會寫進 `~/.aether/constellation.yaml`（使用者層、重裝不會被洗的星圖；可用 `AETHER_CONSTELLATION` override）。repo 裡的 `aether/constellation.yaml` 只是範例/種子。id 會被消毒（小寫、非 `[a-z0-9_]`→`_`），`register`/`observatory` 一致。
