...
Register it (per project, with a DISTINCT `--identity` — not the project's Observatory id):
註冊（每個專案一個，`--identity` 要與該專案 Observatory id 不同以免 inbox 衝突）：

```bash
# user scope (personal, not committed to the repo):
claude mcp add aether -e AETHER_REDIS_DB=0 -- \
    python3 /ABS/PATH/aether/mcp_server.py --identity event_storming_tool-mcp

# …or project scope: copy aether/mcp.example.json into the project's .mcp.json
```

Then in Claude Code (opened in EventStormingTool) just say:
*"問 genesis：你的 BundleParser 需要哪些 SpecBundle 欄位？"* — it calls `aether_ask`,
then `aether_poll`, and reports genesis's grounded answer. See `aether/mcp.example.json`.
...
| **Total** | **65 passed, 2 gated e2e** | + adversarial validation: 100+ probe cases, 0 confirmed defects |
